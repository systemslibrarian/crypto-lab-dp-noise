 ...
 *    `color-mix()`. Every surface this lab uses to deliver a verdict is one:
 *    `.verdict--ok`, `.verdict--bad` and `.verdict--warn` are each
 *    `color-mix(in oklab, var(--ok|--alarm|--warn) 12%, var(--bg-2))`, and the
 *    refusal row in the budget ledger is `color-mix(in oklab, var(--alarm) 10%,
 *    transparent)`. Those four carry the answer of every exercise on the page —
 *    the differencing attack, the k-anonymity attack, the four prediction
 *    checks, the three bounded-sensitivity decisions, the four exit-challenge
 *    questions, the guarantee readout and every budget refusal. axe files them
 *    all under `incomplete`, so a violations-only gate had measured the contrast
 *    of not one verdict this page renders. The shared top bar's `--cl-ink` is a
 *    `color-mix(in srgb, …)` too.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. That branch is dormant here
 *    and was checked rather than assumed: `opacity` does not appear once in
 *    1431 lines of `style.css`. Every de-emphasis on this page lowers LIGHTNESS
 *    instead (`--text-dim`, `--text-muted`), which is the right way round, and
 *    this walk is what keeps it that way if anyone reaches for `opacity` later.
 *
 ...
 *    boxes are the `.scroller` wrappers `dom.ts` builds around every wide table
 *    — the twelve-person payroll, the k-anonymity equivalence classes, the
 *    released-answers ledger, the deployment parameters, and the data-table
 *    alternative under each of the seven charts — all `overflow-x: auto` around
 *    tables far wider than the box at phone width. `.meter` is the other one,
 *    `overflow: hidden` around a fill bar. Content scrolled past those is not
 ...
 *    is full of SVG: `chart.ts` draws seven figures, each with tick labels, two
 *    axis titles and sometimes a ceiling label, all real `<text>`. None of them
 *    sits on a `<rect>` — the charts paint straight onto the `.card--viz` behind
 *    them — so the sibling walk currently finds nothing to composite. It is
 *    load-bearing all the same, and specifically because of what those figures
 *    are made of: `.chart__gridline` and `.chart__axis` are stroke-only
 *    `<line>`s, and SVG's initial `fill` is black, which `getComputedStyle`
 *    reports for stroke-only geometry too. A naive underlay walk would treat
 *    every gridline as an opaque black rectangle covering whatever it crosses
 *    and invent failures for the tick labels beside them; the `FILLED`
 *    tag filter below is what stops that. Add one background `<rect>` to a chart
 *    and the same walk starts doing real work.
 *
 ...
 * spot is NOT empty, and what is in it carries meaning, so it was measured by
 * hand from real screenshot pixels rather than reasoned about:
 *
 *  - `.path__route[aria-pressed='true'] .path__route-label::after` renders the
 *    literal text ` — selected`. It is the visible counterpart of `aria-pressed`
 *    on the route chooser, which is the single most consequential control on the
 *    page — it decides whether five expert panels exist at all — so this is a
 *    string a sighted reader relies on. It does NOT inherit the label's ink: the
 *    rule sets `color: var(--text-dim)` and `font-weight: 400` against the
 *    `.path__route-label`'s `--text` at 700, so measuring the label does not
 *    measure it. Against the `--accent-soft` fill the pressed route paints:
 *    9.29:1 dark, 10.06:1 light, at 16px — clearing the 4.5:1 it needs.
 *  - `.steps li::marker` is the ordered-list numbering in the exhibit walk-
 *    throughs, painted `var(--accent)` at 16px: 4.90:1 dark, 6.86:1 light.
 *
 * Those are the only two. The select chevrons are not a third: `style.css`
 * draws them as two `linear-gradient` background layers on the `<select>` itself
 * using `currentColor`, so they are the same ink as the select's own text, which
 * IS measured — a background image, not a pseudo-element.
 */
 ...
 * uses. Nothing here is transient: every exhibit computes on the click and the
 * result stays on screen until something else changes it, so there is no
 * styling with a lifetime shorter than a scan. The nearest thing to an
 * exception is not one — `chunked()` paints the sampled histograms across
 * animation frames, but under the reduced motion this gate asserts it takes its
 * synchronous branch and does the whole batch in one go, so a scan can never
 * catch a half-drawn chart.
 *
 ...
 * pass on a page this size, so a whole-page scan would catch it only sometimes,
 * and a sometimes-measured assertion is worse than none.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * This is the largest page in the sweep and the caches matter most here.
     * One pass walks eleven sections, seven SVG charts with their tick labels,
     * the twelve-row payroll table, the k-anonymity equivalence classes, the
     * released-answers ledger, the eight deployment rows, the eight
     * honest-scoping cards and the four-step navigator — several thousand text
     * nodes, every one of which re-walks the same ancestors up to `<body>`.
     * Without the caches the pass re-reads the same computed styles and rects
     * tens of thousands of times, and the gate scans about thirty states per
     ...
     * that is every `.scroller` (`overflow-x: auto`, wrapped by `dom.ts`
     * around each wide table) and `.meter` (`overflow: hidden`).
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. It matters a great deal
      // here: this page ships a "Jargon, unpacked" glossary in markup and
      // `chart.ts` puts a full data-table alternative behind a <details> under
      // every one of its seven figures, so most of the page's tabular content
      // starts inside a shut disclosure. The gate opens each by clicking its
      // <summary> — the route a reader has — rather than setting `.open` from
      // script, which is what the gate this replaces did to every <details> on
      // the page before every one of its scans.
      ...
      //
      // This lab DECLARES the idiom and does not currently use it: `style.css`
      // carries a textbook `.sr-only` block (`position:absolute; width:1px;
      // height:1px; clip:rect(0,0,0,0)`) and no markup or module references the
      // class. So the guard is dormant, but it is one `class="sr-only"` away
      // from load-bearing, and the class it is waiting for is already written.
     ...
     *
     * Some of it is glyphs whose meaning is carried in words right beside them,
     * on the same surface, in the same ink — so measuring the words measures the
     * glyph: `dom.ts`'s `.verdict__icon`, the `✓`/`✗` `.marker` in the
     * k-anonymity equivalence table, the `◆`/`●` local-versus-global `.marker`
     * in the deployments table, the numeral/tick `.path__mark`, the
     * `.chart-swatch` legend rules, and the shared header's two glyph-free
     * `<svg>` marks.
     *
     * THE CHARTS ARE DIFFERENT, and are the reason this note exists. `chart.ts`
     * marks the whole figure `aria-hidden` — the grid `<g>`, both axis titles,
     * every ceiling label, every series `<g>` — because each chart ships a full
     * data-table alternative behind a `<details>`, which is the right call. But
     * the labels are still PAINTED, they still carry the numbers, and both
     * oracles skip every one of them. Seven figures' worth of axis and tick text
     * would otherwise be the largest unmeasured surface on the page. Measured by
     * hand from real screenshot pixels, against the `.card--viz` fill the charts
     * paint onto:
     *
     *   `.chart__tick`         --text-muted  11px   7.71:1 dark / 8.19:1 light
     *   `.chart__axis-title`   --text-dim    12px  10.14:1 dark / 11.26:1 light
     *   `.chart__ceiling-label`--warn        11px  10.96:1 dark /  7.15:1 light
     *   `.chart__marker-label` --text-muted  11px   7.71:1 dark / 8.19:1 light
     *
     * All four clear the 4.5:1 an 11px glyph needs. The strokes those labels
     * annotate were measured with them, against the same fill, for SC 1.4.11's
     * 3:1: `--accent` 4.90 / 6.86, `--series-b` 9.32 / 7.47, `--warn` 10.96 /
     * 7.15. `.chart__gridline` and `.chart__axis` are under the floor and are
     * left there deliberately — they are reference rules, not the data, and the
     * data-table alternative under each figure is the text version 1.4.11 exempts
     * a graphic against.
     */
     ...
     *
     * This lab draws seven SVG figures, so the guard is live rather than
     * theoretical — but it never fires, because `chart.ts` builds every node
     * through a typed `svg()` helper that uses `createElementNS` + `append`
     * rather than by interpolating strings. The usual way this mistake arrives
     * — interpolating a string ARRAY into a template literal, which JS joins
     * with commas and leaves as a run of "," text nodes inside a `<g>` — cannot
     * happen through that helper. It stays because a walk that measured such a
     * node would report a 1:1 failure describing ink that was never laid down,
     * and because `<title>` is the one child `chart.ts` DOES put text into
     * directly: a `<title>` is never rendered, and without this guard its
     * accessible-name string would be measured as if it were painted.
     */
     ...
     * `background-color: var(--bg)` on BOTH `html` and `body` — deliberately, per
     * the comment there, as a longhand rather than the `background` shorthand
     * that axe and WebKit miss — and constrains the height of neither, so the
     * root box already spans the document and the ordinary ancestor walk finds
     * it. The composite is kept because "no-op today" is a property of a
     * `height` declaration nobody has written yet, not of the helper, and
     * because this page runs eleven sections and a dozen screens deep: the day
     * someone writes it, almost everything below the hero would report against a
     * white page that does not exist.
     */
